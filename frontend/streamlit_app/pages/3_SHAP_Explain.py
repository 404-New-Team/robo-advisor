from pathlib import Path
import sys

import pandas as pd
import plotly.express as px
import streamlit as st

sys.path.append(str(Path(__file__).resolve().parents[1]))

from api_client import explain
from reference_data import get_default_tickers, get_universe
from ui import COLOR_SEQUENCE, configure_page, format_percent, format_shap_feature, load_api_data, render_sidebar, shap_summary_from_results


configure_page("추천 이유")

state = render_sidebar()
universe = get_universe()

st.title("추천 이유")

target_options = state["active_tickers"] or get_default_tickers()
target = st.selectbox(
    "확인할 종목",
    target_options,
    index=target_options.index("AAPL") if "AAPL" in target_options else 0,
    format_func=lambda ticker: f"{universe.loc[universe['ticker'] == ticker, 'name'].iloc[0]} ({ticker})",
)
result = load_api_data("추천 이유", explain, state["active_tickers"], target, token=state["access_token"])
shap_df = pd.DataFrame(
    [
        {"요인": format_shap_feature(key), "원본 요인": key, "영향도": value, "방향": "늘림" if value >= 0 else "줄임"}
        for key, value in result["shap_values"].items()
    ],
    columns=["요인", "원본 요인", "영향도", "방향"],
).sort_values("영향도")

cols = st.columns(3)
cols[0].metric("추천 투자 비율", format_percent(result["final_weight"]))
cols[1].metric("늘리는 요인", format_percent(shap_df.loc[shap_df["영향도"] > 0, "영향도"].sum()))
cols[2].metric("줄이는 요인", format_percent(shap_df.loc[shap_df["영향도"] < 0, "영향도"].sum()), delta_color="inverse")

st.write(result["explanation"])

left, right = st.columns([1.05, 1])
with left:
    st.subheader("요인별 영향")
    fig = px.bar(
        shap_df,
        x="영향도",
        y="요인",
        color="방향",
        orientation="h",
        color_discrete_map={"늘림": "#2563eb", "줄임": "#dc2626"},
    )
    fig.update_layout(xaxis_title="영향도", yaxis_title="", legend_title_text="", margin=dict(l=10, r=10, t=20, b=10))
    st.plotly_chart(fig, use_container_width=True, key="shap_feature_chart")
with right:
    st.subheader("전체 종목 영향 요인")
    summary_results = [
        load_api_data("추천 이유 요약", explain, state["active_tickers"], ticker, token=state["access_token"])
        for ticker in (state["active_tickers"] or get_default_tickers())[:6]
    ]
    summary_df = shap_summary_from_results(summary_results)
    summary_fig = px.scatter(
        summary_df,
        x="영향도",
        y="요인",
        color="종목",
        size=summary_df["영향도"].abs(),
        color_discrete_sequence=COLOR_SEQUENCE,
    )
    summary_fig.update_layout(xaxis_title="영향도", yaxis_title="", legend_title_text="", margin=dict(l=10, r=10, t=20, b=10))
    st.plotly_chart(summary_fig, use_container_width=True, key="shap_summary_chart")

st.subheader("상세 값")
st.dataframe(shap_df, use_container_width=True, hide_index=True)
