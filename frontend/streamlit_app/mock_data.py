...
def get_research_response(ticker: str | None = "SPY", query: str | None = None, max_results: int = 5) -> dict:
    ...
    return {
        "status": "success",
        "ticker": ticker,
        "summary": f"{target} 관련 최근 리서치에서 주식 성장 모멘텀은 유지되지만 금리, 달러, 레버리지 변동성 리스크가 동시에 감지되었습니다.",
        "risk_events": [
            {
                "type": "rate_sensitivity",
